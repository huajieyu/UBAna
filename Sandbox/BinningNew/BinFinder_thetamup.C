#define BinFinder_thetamup_cxx
#include "BinFinder_thetamup.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void BinFinder_thetamup::Loop()
{
//   In a ROOT session, you can do:
//      root> .L BinFinder_thetamup.C
//      root> BinFinder_thetamup t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
	...
	double lower_bin = 0.18;
	double upper_bin = 2.5;
  ...
  bool first = true;
	...
	while (!exit_flag) {
        ...
        h_reco_mom_pre_truth->Fill(true_mom);
			...
			if (first && mcs_mom > 0 && mcs_mom < 0.18) {
    ...
    double array[2];
		...
		if (lower_bin > 0.29 && lower_bin < 0.31 && upper_bin > 0.99 && upper_bin < 1.1){
			ostringstream oss;
		...
		if (lower_bin > 1.9) exit_flag = true;
...
void BinFinder_thetamup::DoFit(TH1D * the_histo, std::string name, std::string save_name, double *array, bool save_plot) {
...
void BinFinder_thetamup::SmearingMatrix() {
